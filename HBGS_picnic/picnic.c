...
{
    if (params > 0 && params < PARAMETER_SET_MAX_INDEX) { return 1; }
    PRINT_DEBUG(("INVALID PARAMS\n"));
    return 0;
}

/* Parameter transforms restricted to those we need */
...
/* Parameter names restricted to those we need */
...
/** Parameter set names - edited to restrict to those we need */
int get_param_set(picnic_params_t picnicParams, paramset_t *paramset)
{
    ...
    default:
        PRINT_DEBUG(
          ("Unsupported Picnic parameter set (%d). \n", picnicParams));
        return -1;
    }

    paramset->andSizeBytes =
      numBytes(paramset->numSboxes * 3 * paramset->numRounds);
    paramset->stateSizeBytes = numBytes(paramset->stateSizeBits);
    paramset->seedSizeBytes = numBytes(2 * pqSecurityLevel);
    paramset->stateSizeWords =
      (paramset->stateSizeBits + WORD_SIZE_BITS - 1) / WORD_SIZE_BITS;
    ...
    if (paramset->transform == TRANSFORM_UR) {
        paramset->UnruhGWithoutInputBytes =
          paramset->seedSizeBytes + paramset->andSizeBytes;
        paramset->UnruhGWithInputBytes =
          paramset->UnruhGWithoutInputBytes + paramset->stateSizeBytes;
    }
...
int picnic_keygen(
  picnic_params_t parameters, picnic_publickey_t *pk, picnic_privatekey_t *sk)
{
    ...
    LowMCEnc((uint32_t *)pk->plaintext,
      (uint32_t *)pk->ciphertext,
      (uint32_t *)sk->data,
      &paramset);
...
int picnic_sign(picnic_privatekey_t *sk,
  ...
  size_t *signature_len)
{
    ...
    if (!is_picnic3(sk->params)) {
        ...
        ret = sign_picnic1((uint32_t *)sk->data,
          (uint32_t *)sk->pk.ciphertext,
          (uint32_t *)sk->pk.plaintext,
          message,
          message_len,
          sig,
          &paramset);
        ...
        *signature_len = ret;
        ...
        if (sig == NULL) { return -1; }
        ret = sign_picnic3((uint32_t *)sk->data,
          (uint32_t *)sk->pk.ciphertext,
          (uint32_t *)sk->pk.plaintext,
          message,
          message_len,
...
size_t picnic_signature_size(picnic_params_t parameters)
{
    ...
    if (parameters == Picnic3_L1 || Picnic3_L1t || parameters == Picnic3_L5
        ...
        size_t numTreeValues =
          u * ceil_log2((T + (u - 1)) / u);// u*ceil(log2(ceil(T/u)))

        size_t proofSize =
          paramset.seedSizeBytes
            * ceil_log2(paramset.numMPCParties)// Info to recompute seeds
          + paramset.andSizeBytes
          + paramset.stateSizeBytes// circuit size, size of aux info
    ...
    case TRANSFORM_FS:
        // This is the largest possible FS signature size and would result when
        // no challenges are 0 -- which would require us to include
        // stateSizeBytes for every ZKB round.
        return paramset.numMPCRounds
                 * (paramset.digestSizeBytes + paramset.stateSizeBytes
                    + numBytes(3 * paramset.numSboxes * paramset.numRounds)
                    + 2 * paramset.seedSizeBytes)
               + numBytes(2 * paramset.numMPCRounds) + paramset.saltSizeBytes;
    case TRANSFORM_UR:
        return paramset.numMPCRounds
                 * (paramset.digestSizeBytes + paramset.stateSizeBytes
                    + 2 * numBytes(3 * paramset.numSboxes * paramset.numRounds)
                    + 3 * paramset.seedSizeBytes)
               + numBytes(2 * paramset.numMPCRounds) + paramset.saltSizeBytes;
    default:
...
int picnic_verify(picnic_publickey_t *pk,
  const uint8_t *message,
  size_t message_len,
...
int picnic_write_public_key(
  const picnic_publickey_t *key, uint8_t *buf, size_t buflen)
{
    ...
    if (buflen < bytesRequired) { return -1; }
...
int picnic_read_public_key(
  picnic_publickey_t *key, const uint8_t *buf, size_t buflen)
{
    if (key == NULL || buf == NULL) { return -1; }

    if (buflen < 1 || !is_valid_params(buf[0])) { return -1; }
    ...
    if (buflen < bytesExpected) { return -1; }
...
int picnic_write_private_key(
  const picnic_privatekey_t *key, uint8_t *buf, size_t buflen)
{
    if (key == NULL || buf == NULL) { return -1; }
...
int picnic_read_private_key(
  picnic_privatekey_t *key, const uint8_t *buf, size_t buflen)
{
    if (key == NULL || buf == NULL) { return -1; }
...
int picnic_validate_keypair(
  const picnic_privatekey_t *privatekey, const picnic_publickey_t *publickey)
{
    paramset_t paramset;
    int ret;
    ...
    if (privatekey->params != publickey->params) { return -1; }

    if (!is_valid_params(privatekey->params)) { return -1; }
    ...
    LowMCEnc((uint32_t *)publickey->plaintext,
      (uint32_t *)ciphertext,
      (uint32_t *)privatekey->data,
      &paramset);
    if (memcmp(ciphertext, publickey->ciphertext, paramset.stateSizeBytes)
        != 0) {
        return -1;
    }
...
static void print_signature2(
  const uint8_t *sigBytes, size_t sigBytesLen, picnic_params_t picnic_params)
{
    signature2_t sig;
    char label[50];

    /* The test restricted to those we need */
    if (picnic_params != Picnic3_L1 && picnic_params != Picnic3_L1t
        && picnic_params != Picnic3_L5 && picnic_params != Picnic3_L5t) {
    ...
    for (size_t i = 0; i < params.numOpenedRounds; i++) {
        ...
        printHex(
          "msgs", proofs[c].msgs, params.stateSizeBytes + params.andSizeBytes);
        printf("\n");
    }
  ...
  const uint8_t *sigBytes, size_t sigBytesLen, picnic_params_t picnic_params)
{
    ...
    /* The test is restricted to those we need */
    if (picnic_params == Picnic3_L1 || Picnic3_L1t
        || picnic_params == Picnic3_L5 || picnic_params == Picnic3_L5t) {
