...
#define PICNIC_MAX_PUBLICKEY_SIZE    \
    (2 * PICNIC_MAX_LOWMC_BLOCK_SIZE \
      + 1) /**< Largest serialized public key size, in bytes */
#define PICNIC_MAX_PRIVATEKEY_SIZE   \
    (3 * PICNIC_MAX_LOWMC_BLOCK_SIZE \
      + 2) /**< Largest serialized private key size, in bytes */
#define PICNIC_MAX_SIGNATURE_SIZE \
    209522 /**< Largest signature size, in bytes */
...
typedef struct
{
    ...
    uint8_t plaintext[PICNIC_MAX_LOWMC_BLOCK_SIZE]; /**< The input plaintext
                                                       block to LowMC. */
    uint8_t
      ciphertext[PICNIC_MAX_LOWMC_BLOCK_SIZE]; /**< The encryption of plaintext
...
{
    picnic_params_t
      params; /**< The parameter set used with this private key. */
 ...
 * On success, this is set to the number of bytes written to the signature
 * buffer.
 *
 * @return Returns 0 for success, or a nonzero value indicating an error.
 *
 ...
 *       slightly based on the random choices made during signing.  This
 * function will return a suffcient number of bytes to hold a signature, and the
 *       picnic_sign() function returns the exact number used for a given
 * signature.
 *
 * @see picnic_sign()
 */
 ...
 *
 * @return Returns the number of bytes written, at most
 * PICNIC_MAX_PUBLICKEY_SIZE bytes.
 */
 ...
 *
 * @return Returns the number of bytes written, at most
 * PICNIC_MAX_PRIVATEKEY_SIZE bytes.
 */
 ...
 *
 * @return Returns 0 if the key pair is valid, or a nonzero value indicating an
 * error
 */
...
/** Parse the signature and print the individual parts. Used when creating test
 * vectors */
void print_signature(
  const uint8_t *sigBytes, size_t sigBytesLen, picnic_params_t picnic_params);

/******************************************************************************
 * Adaptations for HBGS test 5
 ******************************************************************************/

size_t picnic_signature_size_test5(picnic_params_t parameters);

int picnic_verify_test5(picnic_publickey_t *pk,
...
/******************************************************************************
 * End of adaptations for HBGS test 5
 ******************************************************************************/
