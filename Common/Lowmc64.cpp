...
void copy_lowmc_state_words64_to_lowmc_state_words64(
  Lowmc_state_words64_ptr dest, Lowmc_state_words_const_ptr src) noexcept
{
    dest[lowmc_state_words64 - 1] = 0;
    std::memcpy(dest, src, Mpc_parameters::lowmc_state_bytes_);
}
...
uint8_t parity64(uint64_t x) noexcept
{
...
*/
    /*
     From: https://graphics.stanford.edu/~seander/bithacks.html#ParityWith64Bits
    */
    uint64_t v = x ^ (x >> 1);
    v ^= v >> 2;
  ...
  Matrix64 const &matrix) noexcept
{
    ...
    for (uint32_t row = 0; row < matrix_rows; ++row) {
        ...
        setBit(reinterpret_cast<uint8_t *>(temp), row, parity64(prod));
    }
  ...
  paramset_t *params) noexcept
{
    ...
    for (uint32_t r = 1; r <= Mpc_parameters::lowmc_rounds_; r++) {
        ...
        substitution(reinterpret_cast<uint32_t *>(ciphertext), params);
